...
# Static files = Files that don't change or get processed by the server. They're served directly as-is.
# Jinja2 = A templating engine that creates dynamic HTML pages
app.mount("/static", StaticFiles(directory=str(BASE_DIR / "static")), name="static")
templates = Jinja2Templates(directory=str(BASE_DIR / "templates"))

# This configures CORS (Cross-Origin Resource Sharing) middleware in FastAPI to handle browser security restrictions.
# What it does:

# allow_origins=["*"] - Allows requests from any domain/origin
# allow_credentials=True - Permits cookies/auth headers in cross-origin requests
# allow_methods=["*"] - Allows all HTTP methods (GET, POST, PUT, etc.)
# allow_headers=["*"] - Allows all request headers

# Why needed: Browsers block requests from one domain to another by default. Without CORS, your frontend (e.g., localhost:3000) can't call your API (e.g., localhost:8000).
# Security note: Using ["*"] everywhere is very permissive - fine for development, but in production you should specify exact origins like ["https://yourdomain.com"].

app.add_middleware( # for authentication
    CORSMiddleware,
    allow_origins=["*"],
...
async def analyze_document(file: UploadFile = File(...)) -> Any:
    try:
        ...
        return JSONResponse(content=result) # we return result in JSON response in UI
...
# ---------- CHAT: INDEX ----------
@app.post("/chat/index")
async def chat_build_index( # these are the values which we get from UI
    ...
    try:
        ...
        # this is my main class for storing data into VDB
