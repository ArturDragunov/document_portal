...
class FaissManager:
    def __init__(self, index_dir: Path, model_loader: Optional[ModelLoader] = None):
        ...
        self.meta_path = self.index_dir / "ingested_meta.json" # keep all metadata
        self._meta: Dict[str, Any] = {"rows": {}} ## this is dict of rows
        
        if self.meta_path.exists(): # if path exists
            try:
                # load it if already there or initialize empty one 
                self._meta = json.loads(self.meta_path.read_text(encoding="utf-8")) or {"rows": {}} 
            except Exception:
                self._meta = {"rows": {}} # init the empty one if does not exists
        

        self.model_loader = model_loader or ModelLoader() # either take provided or default one
        self.emb = self.model_loader.load_embeddings()
        self.vs: Optional[FAISS] = None # vector store
        
    def _exists(self)-> bool: # if both are in place, then True 
        return (self.index_dir / "index.faiss").exists() and (self.index_dir / "index.pkl").exists()
    

# Input:
# text: the document text.
# md: metadata dict (like {"source": "file1.txt", "row_id": 42}).

# Logic:
# If metadata has a source (or file_path), the fingerprint is derived from it:
# Example: file1.txt::42
# This means “row 42 of file1.txt”.
# If no source is given, fallback to hashing the text content:
# Example: sha256("The quick brown fox...") → long hex string.

# 👉 Purpose: Two documents from the same file/row get the same fingerprint.
# Otherwise, unique content yields a unique hash.
    @staticmethod # remove duplicate data
...
class ChatIngestor:
    def __init__( self,
        ...
        except Exception as e: # we traceback last execution without sys
